    ...
    IEnumerator GenerateAsync()
    {
        ...
        try
        {
            ...
            using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
            {
                ...
                if(webcam)
                {
                    Texture2D webcamimg = new Texture2D(webcam.webCam.width, webcam.webCam.height);
                ...
                else if(cam)
                {
                    ...
                    Texture2D CamImage = new Texture2D(cam.GetComponent<Camera>().targetTexture.width, cam.GetComponent<Camera>().targetTexture.height);
                    CamImage.ReadPixels(new Rect(0, 0, cam.GetComponent<Camera>().targetTexture.width, cam.GetComponent<Camera>().targetTexture.height), 0, 0);
                    CamImage.Apply();
